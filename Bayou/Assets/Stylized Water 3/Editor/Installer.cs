...
namespace StylizedWater3
{
    ...
    public static class Installer
    {
        ...
        private static bool PerformedStartupCheck
        {
            get => SessionState.GetBool("SW3_Installer_Started", false);
            set => SessionState.SetBool("SW3_Installer_Started", value);
        }
        ...
        {
            if(PerformedStartupCheck == false) Initialize();
            PerformedStartupCheck = true;
        }
        ...
        public static void Initialize()
        {
            ...
            SetupItem unityVersion = new SetupItem(AssetInfo.VersionChecking.GetUnityVersion());
            {
                ...
                else
                {
                    ...
                    //Too risky
                    if (AssetInfo.VersionChecking.ltsVersion == false)
                    {
                        unityVersion.state = MessageType.Warning;
                        unityVersion.description = "Non-LTS versions of Unity are not recommended nor supported." +
                                                   "\n\n" +
                                                   "They are only updated for a few months, before being abandoned (despite being labeled as \"Supported\")." +
                                                   "\n\n" +
                                                   "Updating to or staying on an LTS version is strongly recommended.";
                    }
            ...
            Extension[] extensions = Extension.installed;
            foreach (var extension in extensions)
            {
                //Debug.Log($"Extension {extension.name} has {extension.packageDependencies.Length} dependencies: {string.Join(", ", extension.packageDependencies)}");
                
                foreach (var dependency in extension.packageDependencies)
                {
                    if (IsInstalled(dependency) == false)
                    {
                        PackageInfo packageInfo = GetPackage(dependency);
                        
                        string requiredVersion = dependency.IndexOf('@') >= 0 ? "v"+ dependency.Substring(dependency.IndexOf('@') + 1) : "";
                        SetupItem extensionDependency = new SetupItem($"Missing \"{packageInfo.displayName}\" {requiredVersion} package");
                        {
                            extensionDependency.state = MessageType.Error;
                            extensionDependency.description = $"The <b>{extension.name}</b> extension requires the package for all of its features. Please install it from the Package Manager";
                            
                            extensionDependency.actionName = "Install";
                            extensionDependency.action = () =>
                            {
                                EditorUtility.DisplayDialog("Warning","This package was required to be installed <before> importing this asset." +
                                                                      "\n\n" +
                                                                      "Data saved on components that used this package, or asset files depending on it will have been lost or broken." +
                                                                      "\n\n" +
                                                                      "To remedy this, reimport the asset from the Package Manager." +
                                                                      "\n\n" +
                                                                      "In the future, be sure to read the documentation setup guide first.", "Ok");
                                InstallPackage(dependency);
                                extensionDependency.state = MessageType.None;
                            };
                            
                            AddItem(extensionDependency);
                        }
                    }
                }
            }
            ...
            */
            ...
            SetupItem canvasses = new SetupItem("World Space Canvas");
            {
#if UNITY_6000_4_OR_NEWER
                Canvas[] canvi = Object.FindObjectsByType<UnityEngine.Canvas>();
#else
                Canvas[] canvi = Object.FindObjectsByType<Canvas>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
#endif

                int worldSpaceCanvasses = 0;
                for (int c = 0; c < canvi.Length; c++)
                {
                    Canvas canvas = canvi[c];

                    if (canvas.renderMode == RenderMode.WorldSpace && canvas.sortingOrder == 0) worldSpaceCanvasses++;
                }

                if (worldSpaceCanvasses > 0)
                {
                    canvasses.state = MessageType.Warning;
                    canvasses.description = $"{worldSpaceCanvasses} canvas(es) are set to render in world space, and have their sorting layer set to 0.\n\n" +
                                            $"This may cause water to render over them. Set the layer order to +1 to avoid this.";
        ...
        public static PackageInfo GetPackage(string packageName)
        {
            ...
            {
                Debug.LogError($"Failed to retrieve {packageName} package from Package Manager...");
                return null;
            }
        ...
        private static void ParsePackageId(string packageId, out string packageName, out string minimumVersion)
        {
            packageName = packageId;
            minimumVersion = string.Empty;

            int versionSeparatorIndex = packageId.IndexOf('@');
            if (versionSeparatorIndex < 0)
            {
                return;
            }

            packageName = packageId.Substring(0, versionSeparatorIndex);
            minimumVersion = packageId.Substring(versionSeparatorIndex + 1);
            
            //Debug.Log($"Parsed package id {packageId} into {packageName} v{minimumVersion}");
        }
        
        public static bool IsInstalled(string packageId)
        {
            ParsePackageId(packageId, out string packageName, out string minimumVersion);
            ...
            //Debug.Log($"Checking if package {packageName} v{minimumVersion} is installed");
            ...
            foreach (var package in request.Result)
            {
                ...
                {
                    //Debug.Log($"{packageName} is installed (no minimum version specified)");
                    
                    return true;
                }

                var meetsVersionReq = IsVersionGreaterOrEqual(package.version, minimumVersion);

                if (meetsVersionReq)
                {
                    //Debug.Log($"{packageName} {minimumVersion} is installed...");
                    return true;
                }
            }
        ...
        private static bool IsVersionGreaterOrEqual(string currentVersion, string minimumVersion)
        {
            static string NormalizePackageVersion(string version)
            {
            ...
            Version installed = new Version(NormalizePackageVersion(currentVersion));
            Version minimum = new Version(NormalizePackageVersion(minimumVersion));
            
            //Debug.Log($"Checking if {installed}(installed) >= {minimum}(minimum)...");
