...
namespace StylizedWater3
{
    public class StylizedWaterEditor : Editor
    {
        ...
        {
            var fileName = "New Water Mesh.watermesh";
        ...
        public class MaterialSelector
        {
            ...
            public MaterialSelector(params string[] guids)
            {
                this.guids = guids;
            }

            public void ShowDropDown(Action<Material> onMaterialPicked)
            {
                GenericMenu menu = new GenericMenu();

                void PickMaterial(Material mat)
                {
                ...
                if (guids.Length == 0)
                {
                    menu.AddDisabledItem(new GUIContent("No materials found"));
                }
                for (int i = 0; i < guids.Length; i++)
                {
                    ...
                    if (mat)
                    {
                        string name = mat.name.Replace("StylizedWater3_", string.Empty);
                        menu.AddItem(new GUIContent(name), false, () => PickMaterial(mat));
                    }
                }
        ...
        public static bool RiverModelerInstalled()
        {
            //Checking for the Extension.RiverModeler.cs meta file
            string path = AssetDatabase.GUIDToAssetPath("3d7f93a13b8e47319bd4d14cededbf86");
            return AssetDatabase.LoadMainAssetAtPath(path);
        }
