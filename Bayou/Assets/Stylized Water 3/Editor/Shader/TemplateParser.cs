    ...
    public static class TemplateParser
    {
        ...
        public static string CreateShaderCode(string templatePath, ref string[] lines, WaterShaderImporter importer, bool tessellation = false)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (Matches("%open_xr_keywords%"))
                {
                    #if META_XR_SDK
                    AddLine($"#include \"Packages/com.meta.xr.sdk.core/Shaders/EnvironmentDepth/URP/EnvironmentOcclusionURP.hlsl\"");
                    AddLine("#pragma multi_compile _ HARD_OCCLUSION SOFT_OCCLUSION");
                    AddLine("#if HARD_OCCLUSION || SOFT_OCCLUSION");
                    AddLine("#define META_XR");
                    AddLine("#endif");
                    #endif
                ...
                {
                    if (importer.settings.forceSimpleShadingOnHardware != 0)
                    {
                        var conditions = new List<string>();

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.Mobile))
                            conditions.Add("defined(SHADER_API_MOBILE)");

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.MobileVR))
                            conditions.Add("(defined(SHADER_API_MOBILE) && defined(STEREO_MULTIVIEW_ON))");

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.Switch1))
                            conditions.Add("defined(SHADER_API_SWITCH)");

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.Switch2))
                            conditions.Add("defined(SHADER_API_SWITCH2)");

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.MacOS))
                            conditions.Add("defined(SHADER_API_METAL)");

                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.Web))
                            conditions.Add("(defined(UNITY_PLATFORM_WEBGL) || defined(SHADER_API_WEBGPU))");
                        
                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.PS4))
                            conditions.Add("defined(SHADER_API_PS4");
                        
                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.PS5))
                            conditions.Add("defined(SHADER_API_PS5");
                        
                        if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.XboxOne))
                            conditions.Add("defined(SHADER_API_XBOXONE");

                        if (conditions.Count > 0)
                        {
                            AddLine($"#if {string.Join(" || ", conditions)}");
