...
namespace StylizedWater3
{
    public partial class MaterialUI : ShaderGUI
    {
        ...
        private MaterialProperty _SlopeFoamTex;
        ...
        private bool riverModelerInstalled;
        ...
        private void FindProperties(MaterialProperty[] props, Material material)
        {
            ...
            _DepthOffset = FindProperty("_DepthOffset", props);
            ...
            _RiverModeLegacy = FindProperty("_RiverModeLegacy", props);
            ...
            _SlopeFoamTex = FindProperty("_SlopeFoamTex", props);
        ...
        private void OnEnable(MaterialEditor materialEditorIn)
        {
            ...
            sections.Add(generalSection = new UI.Material.Section(materialEditorIn,"GENERAL", new GUIContent("General", UI.Icons.General)));
            sections.Add(renderingSection = new UI.Material.Section(materialEditorIn,"RENDERING", new GUIContent("Rendering", UI.Icons.Rendering)));
            sections.Add(lightingSection = new UI.Material.Section(materialEditorIn,"LIGHTING", new GUIContent("Lighting", UI.Icons.Lighting)));
            sections.Add(colorSection = new UI.Material.Section(materialEditorIn,"COLOR", new GUIContent("Color", UI.Icons.Color, "Controls for the base color of the water and transparency")));
            sections.Add(normalsSection = new UI.Material.Section(materialEditorIn,"NORMALS", new GUIContent("Normals", UI.Icons.Normals,"Normal maps represent the small-scale curvature of the water surface. This is used for lighting and reflections")));
            sections.Add(underwaterSection = new UI.Material.Section(materialEditorIn,"UNDERWATER", new GUIContent("Underwater", UI.Icons.Underwater,"Pertains the appearance of anything seen under the water surface. Not related to any actual underwater rendering")));
            sections.Add(lightReflectionSection = new UI.Material.Section(materialEditorIn,"LIGHT_REFLECTIONS", new GUIContent("Light Reflections", UI.Icons.LightReflections, "Realtime specular reflection highlight from directional, point and spot lights. ")));
            sections.Add(environmentReflectionSection = new UI.Material.Section(materialEditorIn,"ENVIRONMENT_REFLECTIONS", new GUIContent("Environment Reflections", UI.Icons.EnvironmentReflections, "Reflections from reflection probes, planar- and screen-space reflections.")));
            sections.Add(foamSection = new UI.Material.Section(materialEditorIn,"FOAM", new GUIContent("Surface Foam", UI.Icons.SurfaceFoam)));
            sections.Add(intersectionSection = new UI.Material.Section(materialEditorIn,"INTERSECTION", new GUIContent("Intersection Foam", UI.Icons.IntersectionFoam,"Draws a foam effects on opaque objects that are touching the water")));
            sections.Add(wavesSection = new UI.Material.Section(materialEditorIn,"WAVES", new GUIContent("Waves", UI.Icons.Waves, "Parametric gerstner waves, which modify the surface curvature and animate the mesh's vertices")));
            ...
            riverModelerInstalled = StylizedWaterEditor.RiverModelerInstalled();
        ...
        private void MaterialChanged(Material material)
        {
            ...
            material.SetTexture("_SlopeFoamTex", _SlopeFoamTex.textureValue);
        ...
        private void DrawGeneral()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(generalSection.anim.faded))
            {
                ...
                {
                    DrawShaderProperty(_RiverModeLegacy, new GUIContent("Legacy behaviour", "Enable if the material is not used by a mesh created using the River Modeler extension." +
                        "\n\n" +
                        "River Modeler will bake in the slope weights into the mesh. If not available, the slope can be calculated in realtime in the shader"), 1);
                    
                    if (_RiverModeLegacy.floatValue > 0.5)
                    {
                        UI.DrawNotification(riverModelerInstalled, "River Modeler is installed. Legacy behaviour should be disabled if this material is used with the tool!", MessageType.Warning);
        ...
        private void DrawRendering(Material material)
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(renderingSection.anim.faded))
            {
                ...
                {
                    EditorGUILayout.Space();
                    if (importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.Mobile) || importer.settings.forceSimpleShadingOnHardware.HasFlag(WaterShaderImporter.WaterShaderSettings.TargetHardware.MobileVR))
                    {
                ...
                DrawShaderProperty(_DepthOffset, new GUIContent("Depth Offset", "Use this to avoid Z-Fighting issues with the underlying geometry." +
                    "\n" +
        ...
        private void DrawColor()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(colorSection.anim.faded))
            {
                ...
                if (_ShadingMode.floatValue == 1 || _ShadingMode.hasMixedValue) //Advanced shading
                {
                    ...
                    if (_RefractionOn.floatValue == 0) EditorGUILayout.HelpBox("Requires the Refraction feature to be enabled (Underwater section)", MessageType.None);
                ...
                DrawShaderProperty(_VertexColorTransparency, new GUIContent("Vertex color transparency (G)", "The Green vertex color channel adds transparency to the water, making it appear invisible." +
                    "\n\n" +
                    "If this material is applied to a mesh flooded with a white vertex color it will appear 100% invisible."));
            ...
            if (EditorGUILayout.BeginFadeGroup(normalsSection.anim.faded))
            {
                ...
                if (_NormalMapOn.floatValue > 0f || _NormalMapOn.hasMixedValue)
                {
                    ...
                    if (_DistanceNormalsOn.floatValue > 0 || _DistanceNormalsOn.hasMixedValue)
                    {
                        ...
                        DrawShaderProperty(_DistanceNormalsTiling, "Tiling");
        ...
        private void DrawUnderwater()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(underwaterSection.anim.faded))
            {
                ...
                DrawShaderProperty(_CausticsOn, new GUIContent("Enabled", "Caustics are normally a complex optical effect, created by light passing through a surface and refracting." +
                ...
                DrawShaderProperty(_RefractionOn, new GUIContent("Enabled", "Simulates how the surface behind the water appears distorted, because the light passes through the water's curvy surface"));
                ...
                if (_RefractionOn.floatValue == 1f || _RefractionOn.hasMixedValue)
                {
                    ...
                    DrawShaderProperty(_RefractionStrength, new GUIContent("Strength", "Note: Distortion strength is influenced by the strength of the normal map texture"), 0);
        ...
        private void DrawFoam()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(foamSection.anim.faded))
            {
                ...
                if (_FoamOn.floatValue > 0 || _FoamOn.hasMixedValue)
                {
                    ...
                    DrawShaderProperty(_VertexColorFoam, new GUIContent("Vertex color painting (A)", "Enable the usage of the vertex color Alpha channel to add foam"));
                    ...
                    if (_RiverModeOn.floatValue > 0 || _RiverModeOn.hasMixedValue)
                    {
                        EditorGUILayout.LabelField("River Slopes", EditorStyles.boldLabel);
                        DrawTextureSelector(_SlopeFoamTex, ref foamTextures);
