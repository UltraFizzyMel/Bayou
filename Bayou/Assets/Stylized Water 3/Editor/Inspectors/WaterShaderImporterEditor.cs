...
namespace StylizedWater3
{
    ...
    public class WaterShaderImporterEditor : ScriptedImporterEditor
    {
        ...
        private SerializedProperty forceSimpleShadingOnHardware;
        ...
        public override void OnEnable()
        {
            ...
            forceSimpleShadingOnHardware = settings.FindPropertyRelative("forceSimpleShadingOnHardware");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(forceSimpleShadingOnHardware, new GUIContent("Force simple shading", forceSimpleShadingOnHardware.tooltip));
            ...
            if (type.intValue == (int)WaterShaderImporter.WaterShaderSettings.ShaderType.WaterSurface)
            {
                ...
                if (curvedWorldInstalled)
                {
                    ...
                    EditorGUILayout.HelpBox("Curved World integration must be manually activated through minor code changes, see documentation.", MessageType.Info);
                }
                
                #if META_XR_SDK
                using (new EditorGUILayout.HorizontalScope())
                {
                    EditorGUILayout.LabelField("Meta XR", GUILayout.MaxWidth(EditorGUIUtility.labelWidth));
                    using (new EditorGUILayout.HorizontalScope(EditorStyles.textField))
                    {
                        GUI.contentColor = Color.green;
                        EditorGUILayout.LabelField("Installed");
                ...
                EditorGUILayout.HelpBox("Meta's depth shader API will be used to apply environment occlusion.", MessageType.Info);
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.LabelField("River Modeler", GUILayout.MaxWidth(EditorGUIUtility.labelWidth));

                using (new EditorGUILayout.HorizontalScope(EditorStyles.textField))
                {
                    if (riverModelerInstalled)
                    {
                        GUI.contentColor = Color.green;
                        EditorGUILayout.LabelField("Installed");
                    }
