...
{	
	...
	scene.verticalDepth = max(0.0001, DepthDistance(water.positionWS, scene.positionWS, water.waveNormal * normalSign)) - (_DepthOffset * 0.1);
...
float4 ForwardPass(Varyings input, FRONT_FACE_TYPE_REAL vertexFace : FRONT_FACE_SEMANTIC_REAL)
{
	...
	float3 positionWS = input.GetPositionWS();
	...
	#if _RIVER
	//Slope weight stored by River Modeler
	//return float4(input.uv.www, 1);
	water.slope = input.uv.w;
	...
	water.intersection = SampleIntersection(_IntersectionNoise, sampler_IntersectionNoise, uv.xy + (offsetVector * _IntersectionDistortion), (TIME * -_Direction),
	...
	#if _WAVES && !_DISABLE_DEPTH_TEX && !_RIVER
	...
	#else
	float foamSlopeMask = saturate((water.slope * _SlopeFoam));
	#endif
	...
	#else
	//Defensive init: ensures lighting never reads an undefined normal (important on WebGPU)
	water.diffuseNormal = water.vertexNormal;
	#endif
	...
	//WebGPU safety: APV isn't entirely supported and will propagate NaNs into bakedGI. Resulting in the water turning invisible.
	#if defined(SHADER_API_WEBGPU) && (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
	//Fallback to SH/Light probes
...
)
{
	...
	#if defined(META_XR)
	META_DEPTH_OCCLUDE_OUTPUT_PREMULTIPLY_WORLDPOS(input.GetPositionWS(), outColor, 0);
	#endif

	#ifdef _WRITE_RENDERING_LAYERS
	//Note: this implementation is for Unity 6.1+ and throws an error in older versions
	//But rendering layers for transparent materials aren't used in any native URP features, so this code path won't normally be active anyway
