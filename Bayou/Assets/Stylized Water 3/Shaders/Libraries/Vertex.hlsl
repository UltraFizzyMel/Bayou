...
{	
	...
	{
		#if REQUIRES_TANGENT_TO_WORLD
		return float3(normalWS.w, tangent.w, bitangent.w);
		#else
		return positionWS;
		#endif
	}
...
Varyings LitPassVertex(Attributes input)
{
	...
	output.uv.w = input.uv0.z; //River modeler slope weight
	...
	#if !defined(DISPLACEMENT_BUFFER_PER_PIXEL) //Displacement will be calculated per pixel
#if _WAVES
	float2 uv = GetSourceUV(input.uv0.xy, positionWS.xz, _WorldSpaceUV);
