...
TEXTURE2D(_SlopeFoamTex);
SAMPLER(sampler_SlopeFoamTex);
	...
	bool slopeFoamOn, bool distanceFoamOn, float distanceStart, float distanceEnd, float distanceTiling)
{
	...
	if(slopeFoamOn)
	{
		...
		const half2 slopeFoam = SampleFoamLayer(TEXTURE2D_ARGS(_SlopeFoamTex, sampler_SlopeFoamTex), slopeUV, tiling, time, speed * slopeSpeed, subTiling, subSpeed * slopeSpeed);
...
float SampleIntersection(TEXTURE2D_PARAM(noiseTex, samplerName), float2 uv, float2 time, float tiling, float gradient, float falloff, float speed, half rippleDistance, float rippleStrength, float rippleSpeed, float clipping, bool sharp)
{
	...
	half noise2 = 0;
	#if _ADVANCED_SHADING
	float2 offset2 = (time.xy * speed);
	#if _RIVER
	offset2 = -offset2;
	#endif
	noise2 = SAMPLE_TEXTURE2D(noiseTex, samplerName, (nUV * 0.8) - offset2).r;
	#endif
