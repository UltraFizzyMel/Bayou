...
void ApplyFog(inout float3 color, float fogFactor, float4 screenPos, float3 positionWS, float vFace) 
{
	...
	{
		half linearDepth = Linear01Depth(SampleSceneDepth(normalizedUV), _ZBufferParams);
		foggedColor.rgb = ApplyFogAndVolumetricLights(color.rgb, normalizedUV, positionWS, linearDepth);
