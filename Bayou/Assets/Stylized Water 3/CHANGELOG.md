3.3.1 (September 16th 2026)

Fixed:
- Shader compile error when using the Meta XR SDK.
- Script compile error if the "Unity Web Request Texture" module was disabled in a project.
- Script compile error in older 6.1-6.2 versions due to Switch2 not existing during their release.

3.3.0 (September 8th 2026)
Support has been extended to Unity 6.6. This update includes many internal changes for extensions, an requires updating them as well. 

< Update the Dynamic Effects and Underwater Rendering extensions the latest version as well! >

Added:
- Icons for every section in the Material UI
- Hub Window now displays all the available demo scenes (including for extensions)
- Warning to installer if a World-Space canvas has a sorting order of 0, which may cause water to render over it.
- Depth pre-pass option. Renders the depth of the water surface in a texture (_WaterDepthTexture). This may be used in custom effects and can be enabled on the Render Feature.
- "Sample Water Position" sub-graph that uses the above depth texture to sample the water surface position.
- Experimental support for Meta XR occlusion functionality + using depth from the pass-through environment (requires the 'com.meta.xr.sdk.core' package).
- Depth Offset material parameter, may be used to mitigate z-fighting (eg. rivers above a terrain surface).

Changed:
- The "Force Simple Shading On Mobile" option on the shader is now a dropdown menu that's specifies multiple platforms.
- River materials now have a "Legacy Behaviour" toggle. This should be enabled for river materials not being used with the River Modeler tool.
- Surface Foam now has a separate texture slot for slope-based foam.
- Updated particle effects to use a modified quad mesh so that they don't appear darker when facing away from the sun.

Changed:
- Material UI: Tiling parameter for distance normals is now a slider.

3.2.8 (June 22nd 2026)
